    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[10000] p-4" onClick={onClose}>
      <div className="bg-white rounded-[24px] max-w-[90%] w-full max-h-[85vh] overflow-y-auto shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-6 text-center border-b border-slate-200">
          ...
          <h2 className="text-[38px] font-bold text-slate-800 mb-2">Modo de Emergencia</h2>
          <p className="text-[18px] text-slate-600">Seleccioná una opción para recibir ayuda rápidamente.</p>
        </div>
            ...
            {emergencyOptions.map((option) => {
              ...
              return (
                <button
                  ...
                  className="w-full h-[130px] bg-white rounded-xl border-2 border-slate-200 shadow-md hover:shadow-lg transition-all duration-300 flex items-center px-6 hover:border-[#43A1F2]"
                >
                  <div className={`p-4 rounded-xl ${option.iconColor} bg-white shadow-sm`}>
                    <Icon size={32} />
                  </div>
                  <div className="flex-1 ml-4 text-left">
                    <h3 className="text-xl font-bold text-slate-800 mb-1">{option.title}</h3>
                    <p className="text-base text-slate-600">{option.description}</p>
                  </div>
        ...
        <div className="p-6 border-t border-slate-200">
          <button
            onClick={onClose}
            className="w-full py-4 bg-white border-2 border-red-500 text-red-500 rounded-xl font-bold hover:bg-red-50 transition-colors text-lg"
          >
            Cancelar
          </button>
        </div>
